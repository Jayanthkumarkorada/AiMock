...
const CandidateInformation = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      // Add basic form fields with trimmed values
      ...
      formDataToSend.append('birth_Date', new Date(formData.birth_Date).toISOString());
      formDataToSend.append('address', formData.address.trim());
      formDataToSend.append('graduation', formData.graduation.trim());
      formDataToSend.append('role', formData.role.trim());
      
      // Handle skills array - ensure it's a valid array
      const skillsArray = Array.isArray(formData.skill) ? formData.skill : [];
      const validSkills = skillsArray.filter(skill => typeof skill === 'string' && skill.trim());
      formDataToSend.append('skill', JSON.stringify(validSkills));
      ...
      if (formData.profile instanceof File) {
        if (!formData.profile.type.startsWith('image/')) {
          toast.error('Profile must be an image file');
          return;
        }
        if (formData.profile.size > 5 * 1024 * 1024) {
          toast.error('Profile image size must be less than 5MB');
          return;
        }
        formDataToSend.append('profile', formData.profile);
      } else if (typeof formData.profile === 'string' && formData.profile) {
        formDataToSend.append('profile', formData.profile);
      }
      ...
      console.log('Submitting candidate information...');
      console.log('URL:', url);
      ...
      if (response.data?.success) {
        toast.success(isEdit ? 'Information updated successfully!' : 'Information submitted successfully!');
